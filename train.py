            ...
            with self.ctx:
                with torch.no_grad():
                    if not self.is_ddp_training:
                        outputs = self.model(**{key: value.to(self.gpu_id) for key, value in batch.items()})
                    else:
                        outputs = self.model(**batch)
...
def load_pretrained_model(local_rank, model_path: str = ""):
    ...
    model = LoraModelForCasualLM(config=lora_config, base_model=model)
