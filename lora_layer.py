...
class Linear(nn.Linear, LoraLayer):
    ...
    def merge(self):
        ...
        if self.merged:
            ...
            return
        if self.r[self.active_adapter] > 0:
            # Merge the LoRA parameters by adding the product of lora_B weights and lora_A weights (after transposing 
            ...
            self.weight.data += (
                ...
                * self.scaling[self.active_adapter]
            )  # Merged weights calculation
            self.merged = True  # Set the merged flag
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            # Apply LoRA layers if active
            result += (
                self.lora_B[self.active_adapter].weight @ self.lora_A[self.active_adapter].weight @ x.T
